...
2. **HEAD is right.** `git -C <wt> rev-parse HEAD` equals the base tip resolved **locally** (`git -C <repo> rev-parse <base>`) — no fetch, no freshness count, this worktree being a throwaway and the config's reader what is under test. The full fetch-and-compare belongs at **dispatch** (`skills/execute/references/worktrees-and-branches.md`).
