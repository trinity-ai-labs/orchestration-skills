...
| Script | Role |
|---|---|
| `enqueue-gate.mjs` | Drops a durable ticket for one gate — an implementer's, after pushing and opening its draft PR, or a dispatcher's own integration gate on a tree that has no PR |
...
**`prNumber`/`prUrl` are OPTIONAL, because a tree worth gating does not always have a PR.** The case that needs it is the dispatcher's mid-arc integration gate (`skills/execute/SKILL.md`'s *Gate the integrated whole*): a merge whose `^2` diff comes back non-empty has produced a tree no gate has run, and that tree exists *between* slice merges, when the epic → integration PR does not exist and will not for some time. A ticket carrying no PR is claimed, gated, logged and settled into `done/` exactly like any other, and the only step it skips is the report — so its verdict lives on the ticket and nowhere else, which is precisely the state invariant 7 already builds for. *The failure this prevents: with the PR fields required, that gate cannot be enqueued at all, so the one gate whose tree no per-slice gate ever saw is the one run by hand — no log, no ticket, no ledger entry, and a verdict that survives only as an exit status in the terminal of whoever ran it.*

**Record the absence at enqueue as a fact ON the ticket — undeliverable by construction — never leave it to be inferred from two missing fields.** Two later readers need it and neither can reconstruct it safely: invariant 8's prune has to tell a ticket nobody will ever report from one whose report failed, and *Reporting* below has to keep "settled, no comment" diagnosable. *The failure this prevents: inferring it means reading absent PR fields as "no delivery owed", which is the same move as reading `reported: false` as "safe to delete" — the trap invariant 8 exists to name — and it makes an enqueue that dropped its PR fields by accident indistinguishable from one that never had any.*

**A runner scaffolded before this change must REJECT a PR-less enqueue and exit non-zero — never accept it and gate a tree whose verdict has nowhere to go.** This design is authored *into* a project and evolves there, so a required argument becoming optional is a contract change reaching runners this reference does not hold. That is the same rollout shape as invariant 9's `--status`, and it takes the same treatment: the older side makes the gap legible by refusing, rather than by doing the adjacent thing. A runner that took the ticket and then tried to comment on a PR that was never named would spend a full serialized gate and file the result as a delivery failure — the one classification that escalates to a human. *The failure this prevents: rejection costs the caller one clear error and a hand-run gate; silent acceptance costs a gate run and leaves a flagged ticket in the ledger that no retry can ever clear.* **That hand-run gate is a sanctioned route rather than a caller left improvising, and it is written down where the caller stands** — `skills/execute/SKILL.md`'s *Gate the integrated whole*, at *A runner scaffolded before the PR-less ticket REJECTS it*. Say so here because this is the only place the refusal is specified: a rejection that costs the caller a clear error still strands it if the error is the last thing anyone wrote about the case, and the dispatcher reading it is in a project whose queue works, where every other fallback in that file reads as not applying to them.

**A consumer that scopes on the ticket scopes on `branch`, and treats `prNumber` as absent.** `branch` and `worktreePath` are on every ticket; the PR fields are not, so a reader that keys on `prNumber` — or prints it as a handle — silently drops PR-less tickets or labels them with nothing. `skills/execute/SKILL.md`'s *Wait on your own tickets settling — one Monitor over the queue's ledger* is the consumer this reaches: it already scopes on `branch`, so PR-less tickets compose with it unchanged, and the handle it prints is the one thing that needs the branch as its fallback. *The failure this prevents: a watch that silently skips a class of ticket is indistinguishable from a wave that has not settled yet — the same failure the two `--status` bullets in invariant 9 record, arriving through a field that is absent rather than a mode that was never implemented.*
...
It also makes `done/` the one place a **waiter** can watch. Settling is a rename into that directory and the verdict is already on the ticket by then, so a dispatcher waiting for its own tickets watches the ledger rather than the PR — where the verdict only arrives later, and on a failed post does not arrive at all. `skills/execute/SKILL.md`'s *Wait on your own tickets settling — one Monitor over the queue's ledger* is that consumer; it depends on this invariant and on the enqueue-time ticket shape above, and on nothing else the ticket carries, because those are the only fields whose spelling this reference pins. Invariant 8 is what keeps the watch honest at the other end: its predicate prunes on `delivered && older than the retention window`, so a ticket that has just settled is still there when the waiter wakes.

**A PR-less ticket is this same state with delivery permanently not applicable — not a fourth state.** Its verdict is recorded exactly as every other ticket's is; what differs is that no report will ever be attempted, because there is nothing to attempt one against. So for that ticket the ledger is not the copy that survives a failed post, it is the only copy there was ever going to be, and whoever wants the verdict reads it there. *The failure this prevents: modelled instead as "delivery pending, forever", it sits in `done/` looking exactly like a post that keeps failing — the one shape invariant 8 refuses to prune and* Reporting *below flags for a human — so the ledger fills with escalations nobody can act on, each of them a gate that did precisely what it was asked to.*
...
**An undeliverable ticket counts as DELIVERED the moment it settles, and then prunes on the retention window like any other.** The `delivered` half of the predicate asks whether the verdict reached the reader it was owed to, and a ticket enqueued with no PR was owed no report — nothing about it is outstanding and there is nothing for a human to resolve. Take that classification off the flag the enqueue recorded (the ticket shape above), never off the absence of a comment, which is the same inference this invariant already refuses in the other direction. Its reader is inside the window by the same guarantee the waiter above relies on: the dispatcher that enqueued it is watching `done/` for it. *The failure this prevents runs both ways. Read as flagged, every mid-arc integration gate becomes a permanent ledger entry, and the unbounded growth this invariant exists to bound arrives through the one class of ticket that is working perfectly. Read as prunable on age alone, a ticket whose post genuinely failed is swept as soon as it is old enough — the exact deletion the predicate was written to prevent, since on a flagged ticket age is usually evidence that nobody has looked.*
...
**The verdict is a PR comment, in both directions, and the PR stays draft either way.** Green posts a passing comment; red posts the failing tail. For a ticket that has a PR the comment is the only channel — the queue never touches the PR's draft flag, in either direction.
...
**A PR-less ticket is a fourth producer of that state and the only benign one — which is exactly why it has to be READABLE on the ticket rather than inferred.** It settles with a verdict and no comment because there was never a PR to put one on, and from outside the queue that is byte-for-byte the failed post above. The enqueue-time flag from the ticket shape is what separates them, and the difference it carries is the difference between *"the verdict lives here, read it"* and *"a verdict is stranded, go find out why"*. *The failure this prevents: a reconcile pass that treats every commentless settled ticket as a delivery to retry spends its retry cap on tickets that have no destination, then leaves them flagged for a human who arrives to find a gate that did exactly what it was asked to — and while that human is reading those, the flagged pile no longer means what invariant 8 needs it to mean.*
...
A verdict counts as delivered exactly when its comment lands on the PR — or, on a ticket that carries no PR, the moment it settles, since no post is owed and neither retry rule above has anything to act on. There is no second channel to fall back on, which is why the ticket holds the verdict and the reconcile keeps retrying until the comment is there.
