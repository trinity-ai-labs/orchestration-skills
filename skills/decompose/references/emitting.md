...
1. **Rewrite the issue body** into an umbrella overview: the goal, the parallelization plan (waves, conflict map, critical path), **the remainder at shape depth** — each beyond-horizon slice's goal, area, depends-on and why-it-comes-after, exactly as the in-chat *Beyond the horizon* section above carries them — and a **tracked checklist**, which GitHub renders as progress. **A checklist line carries a number only where a sub-issue exists for it**: `- [ ] #<sub>` at the horizon, and a plain `- [ ] <title>` for a beyond-horizon slice, which indexes that slice's entry in the remainder rather than replacing it.
2. **Create one sub-issue per horizon slice** (or per tightly-coupled cluster) carrying that slice's full brief — scope, do-not-touch, depends-on, skill-to-invoke, model hint, verify. **Beyond the horizon the default is a checklist line in the umbrella body and no brief**, and a placeholder sub-issue — which gets no brief either — is the exception on one test: the item needs what a line cannot carry, an assignee of its own or a close of its own. Title each with its wave (e.g. `[W1] <title>`) so the dispatch order is visible at a glance.
