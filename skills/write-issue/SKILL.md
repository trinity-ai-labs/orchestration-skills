...
- **Verify** — what "done" looks like: the behavior, the tests, the gate/check that must pass, the greps that must come back empty. A checkable bar, not a vibe. **A negative in it names what it is measured against** — *unchanged* and *no new X* are claims about a difference, so the baseline is the fork point unless you name a different one, while a grep names the domain it sweeps instead; a bar that names neither is satisfied by whichever end whoever runs it happens to pick (`skills/decompose/SKILL.md` Step 3's `Verify` field carries the argument). **And a bar filed here outlives the arc, so read it against the rest of the body before you file it — as the same three pairs `/pipeline:decompose` reads a slice's fields against** (`skills/decompose/SKILL.md` Step 3's closing check, which is the authoritative statement of them): the bar against the **Constraints**, the bar against what the **Approach** actually asks for, and any requirement in it against the vocabulary the target has to express it with. *The failure this prevents, observed: a requirement the target could not express was written into a slice's brief and, in the same words, into this issue's `Verify` bar. The implementer raised it and the brief was corrected by hand; the issue was not — so a correct PR sat against a bar it deliberately did not meet, and the issue would have closed reading as unmet. A brief is rewritten every cycle and a contradiction in one dies with it; a filed bar is inherited by every slice grounded from that issue and re-read long after the run that would have caught it is over.*
