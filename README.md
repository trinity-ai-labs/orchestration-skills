...
| [`/pipeline:review`](skills/review/SKILL.md) | A caller's own quality pass over the **PR it has just opened**, read against that PR's real diff: one briefed reviewer per dimension it judges the change needs, each spawned at a tier that is named rather than inherited from the caller — standard for a reader over one dimension, higher where that dimension is genuinely hard — each reporting what it ran, all of them weighed by the caller, and the verdict posted back onto that PR as a review anyone can read. **Usually an implementer over its own slice; once per epic it is the orchestrator over the close-out PR**, where the full seven-dimension panel runs between that PR's open and its gate against the combined diff no slice's own pass ever saw, and a fix agent — never the orchestrator, which writes no code — applies what it raises. |
