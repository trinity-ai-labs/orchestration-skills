...
| [`/pipeline:review`](skills/review/SKILL.md) | An implementer's own quality pass over the **PR it has just pushed**, run once its draft PR is open and read against that PR's real diff: one briefed reviewer per dimension it judges the slice needs, each spawned at a tier that is named rather than inherited from the implementer — standard for a reader over one dimension, higher where that dimension is genuinely hard — each reporting what it ran, all of them weighed by the implementer, and the verdict posted back onto that PR as a review anyone can read. |
