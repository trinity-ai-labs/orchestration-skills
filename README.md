...
**Seven skills, and one front door.** `/pipeline:setup` onboards a repo once. After that `/pipeline:co-think` is where work arrives — it settles the shape and routes it, and every command after that is named for you by the pass before it.
...
**`branchingModel` is the one fact no repository states about itself, and several rules used to guess it.** `"trunk"`, `"release"` or `"gitflow"` says which branch work lands on — the repository's default branch, the single live `release/x.y.z`, or `develop` — and the flow reads it instead of inferring. **Absence is a settled answer**: the old inference stands, so a project that never sets it behaves exactly as before. A workspace declares it once for members sharing one branch and one model.

**`epicMerge` reaches exactly one merge, and without `branchingModel` declared it declines to reach it in almost every project.** It governs an epic branch's own collapse back into the integration branch — the branch is scaffolding, cut for one arc and deleted at its end, so a project may prefer one commit per arc to N slice merges plus a merge commit. Every other merge in the flow stays a real merge commit with no opt-out. Even under `"squash"` two conditions must both hold, and one of them is the reason to read the rule before setting the key: **the head branch must not be the integration branch**, which is what the declared model establishes. Undeclared, the helper falls back to asking whether the PR's *base* is the repository's default branch — a proxy that holds only where the integration branch is not also the default branch, which turns out to be the uncommon case. So an undeclared project usually gets no squash at the genuine epic boundary, and the behaviour is otherwise indistinguishable from the option being broken, which is why it is stated here rather than left to be discovered. Both ports compare the key, the value and the branch names case-sensitively, so `"Squash"` and `"EpicMerge"` mean `merge` in bash and PowerShell alike. The other condition, why every unanswerable question falls back to `merge`, and the trade the option makes are all in [`skills/execute/references/worktrees-and-branches.md`](skills/execute/references/worktrees-and-branches.md) → *Mechanics*, which is the one place that argument lives.
