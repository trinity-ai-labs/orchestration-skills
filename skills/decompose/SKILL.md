...
- **Brief** — 2–5 sentences the dispatcher can hand almost verbatim to an implementer: what to build, the existing pattern/file to copy, and the hard boundaries. Include any research-first step.
...
- **Cross-reference, don't auto-close.** Sub-issues reference the umbrella (`#<umbrella>`); the dispatcher closes each as its PR merges (PRs merge into the integration branch, not `main`, so GitHub won't auto-close).
