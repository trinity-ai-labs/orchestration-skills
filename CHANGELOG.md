...
- **A folded item was folded correctly and still acquired an issue number, because the disposition placing it was an unweighted `or`.** `/pipeline:decompose`'s emitting reference offered a beyond-horizon slice "a checklist line **or** a placeholder sub-issue" with no default and no test, and then gated the placeholder on "when you want it tracked" — a preference written in the voice of a default, since *tracked* is self-evidently desirable and a folded item is a beyond-horizon slice by construction, so it met that `or` every time and the branch reading as responsible won. The `or` is now weighted: **the checklist line in the umbrella body is the default, and a placeholder sub-issue is the exception on one test — the item needs what a line cannot carry, an assignee of its own or a close of its own.** A beyond-horizon slice **is already tracked**, the umbrella body being the live remaining plan the loop re-reads every cycle, so *tracked* is not the test and a number bought for tracking alone buys nothing.
- **The rule is *not yet*, never *never*, and both seats now say when the number arrives.** One sub-issue per **horizon** slice was already required in that same file, so a folded item takes one there like any other once the horizon reaches it and it is ground to slice depth. `/pipeline:orchestrate`'s *Forced* verdict carries the matching clause, because that is the seat the loop actually reads as it folds — the emitting reference is read by the pass that writes the umbrella, never by the pass deciding an item belongs in it.
- **An implementer's out-of-fence findings had no scheduled moment to surface, so they surfaced at the handoff, where answering one costs a whole fix agent instead of one message and an edit.** Asking was already the default and the four answers already existed, but the trigger was purely reactive: the implementer asked when it *bumped into* a fence trying to fix something, and nothing told it to go look. The one place enumerating out-of-fence work sat under the handoff heading, the latest point in the run — so which disposition an item got correlated with the moment it was raised rather than with its size or its class. **The sweep is now a named step of the WORK rather than of the handoff: run it once the code is done and the docs are current, and in any case before you push.**
- **Push is the anchor, and the review pass deliberately is not.** Push is the one boundary every slice crosses exactly once and the last moment before the PR and the gate ticket exist, so an answer arriving before it is an edit in a tree still open and one arriving after it is a fix agent dispatched back into that worktree. `/pipeline:review` is the **dispatcher's per-slice call**, so a sweep hung there would inherit that conditionality and vanish on every slice that runs no pass; anchored to push it still catches whatever that pass surfaces, since the pass reads the *uncommitted* diff and the commit comes after it. **Filing did not move** — the handoff still files, and now says in as many words that it records the *answers* and is not where the looking happens. **The dispatcher gets the detector**: a hand-back surfacing an out-of-fence finding for the FIRST time is a sweep that did not run, a fact only the reviewing seat is positioned to notice.
- **The cycle record counted a level and an age, and never a flow.** How many filed items are outstanding and how many cycles the oldest has been say how much is standing and how long it has stood, and nothing about which way the pile is moving — so a level rising by one a cycle reads as noise, and a run that closed ten more than it filed and then swung to filing more than it closes shows nothing at all. The increment comment now also carries **filed this cycle, closed this cycle, and the net, where net is FILED MINUS CLOSED** so a positive net is the backlog growing, with the sign written out where the term is introduced rather than left to be inferred. **No checklist item was added**: item 8 already re-disposes every issue filed out of the arc every cycle, so the rate rides the walk that is already happening.
- **A fifth exit, whose condition is part of the rule rather than a caveat trailing it.** Two consecutive cycles at a net of zero or more **halt the loop only while items the arc filed are still *Adjacent*** — reported like any other halt, with the re-plan recommended. The condition sits inside the rule because unconditioned the exit fires on an arc that has filed nothing at all: 0 − 0 = 0, so a young arc filing nothing and closing nothing would halt on its second cycle, and where nothing is outstanding there is no backlog to transfer.
- **Both halves of the subtraction now name one population and one window, which is where this nearly shipped inverted.** A first draft defined *closed this cycle* as how many of the arc's filed issues "are now closed" — a running total, not a flow — which makes the subtrahend grow forever and reports a backlog shrinking while it grows; and it drew *filed* from the loop's own *Adjacent* verdicts while drawing *closed* from every issue filed out of the arc, so a cycle whose implementers filed three and whose loop filed none scored flat while the level climbed. Both counts now run over **item 8's population** — this loop's own filings and its dispatchers' and implementers' alike — across the window **since the previous increment comment**. The walk that gathers them is told to cover closed issues as well as open, since a closed one has left the re-disposition set and would otherwise be invisible to the count it belongs in.
- **Written at every seat that states one of the three rules or reads its record.** For the fold: `skills/decompose/references/emitting.md` and `skills/orchestrate/references/reconciling.md`'s *Forced* bullet. For the sweep: `skills/execute/references/implementer.md`'s flow line, its out-of-fence section and its handoff paragraph, `skills/execute/references/reviewing.md`'s detector, and `skills/review/SKILL.md`'s *Flagged, out of scope* bullet — the richest concentrated feeder of out-of-fence findings in a run, which already stated the ORDER (the caller's dispatcher before the tracker) and named no moment, so a caller reading only that bullet carried the pile to the handoff. For the rate and the exit: `skills/orchestrate/references/reconciling.md`'s *Rewriting the plan* and the cycle-record sentence the checklist ends on, and `skills/orchestrate/SKILL.md`'s exits paragraph, now five. **The paired doc copies moved in the same commit as the prose they mirror**, since nothing mechanical links them: `docs/hard-rules.md` and `docs/mental-model.md` gain the rate on their counting sentences and the push anchor on their fence rules, `docs/mental-model.md` and `README.md` gain the fifth exit on their halt descriptions. `skills/execute/SKILL.md` took the sweep as a numbered step of its own — its *actions, in order* list says of itself that it carries every step in full, so a named step of the work missing from it is the spine contradicting the reference an implementer loads it beside; the two steps below it renumber, and `AGENTS.md`'s citation of the attribution step follows them. Its follow-up-ownership hard rule was read separately and left alone: it states an **order** — dispatcher before tracker — and imposes no moment, so a push-anchored sweep composes with it. **Four further seats stated the beyond-horizon default and were swept by concept rather than by wording**: `skills/review/SKILL.md`'s *Flagged, out of scope* bullet, the richest concentrated feeder of out-of-fence findings, which named an order and no moment; `skills/decompose/SKILL.md`'s frontmatter and step 4, whose unqualified "one sub-issue per slice" advertised the disposition this release removes in the first thing a reader loads; and the two glossary entries whose glosses said every checklist line links a child. `README.md`'s one-line implementer flow was checked and left alone, since it never described the ask and so has no seat the sweep could be missing from; `skills/write-issue/SKILL.md`'s umbrella bullet was checked and left alone too, because it already gates on independent closure — the same test — and governs phases where the new default governs slices.

## 4.19.0

- **A fence grant's second half was provoked by nothing, and the half that fails is the half the reviewer reads.** The rule told a dispatcher granting a path to write the grant where it outlives the run — a comment on the slice's PR, or on the issue behind it while no PR is open yet, *repeated onto the PR once it opens*. Every other step in this flow is provoked by something: a gate verdict arrives, a hand-back arrives, a tick fires. That repeat was provoked by the dispatcher noticing, on some later turn, a fact nothing re-presents. **Five grants were made across the two slices that shipped the last two releases, and the dispatcher wrote none of them onto a PR.** It wrote each onto the issue and then instructed the *implementer* to restate them in the PR body — which worked, and is not the rule, and is the same silent failure wearing a workaround's clothes. In an earlier wave of three, two reached their PRs only because the dispatcher happened to be writing a review comment at the time, and the third surfaced only when the implementer holding it reported that the grant was recorded nowhere it could see. That is the party the rule exists to protect noticing the rule was not followed.
- **A tick rider was the obvious fix and the evidence runs against it.** A rider fires only if the tick lands between the PR opening and the merge, and in every one of those slices the implementer pushed, opened its draft PR and handed back in the same closing sequence — so no tick ever observed a PR-without-grant window. A rider would have been correct, carried forward in the wakeup prompt, and would still have caught nothing. That is not a tuning problem: the closing sequence is the designed path rather than a fast edge case, so the window a poll watches for is routinely shorter than the poll interval.
- **The write is attached to the REVIEW boundary instead, which is a gate rather than a poll.** *Before you review a PR, post any grant that slice worked under* attaches to an act that is certain, happens exactly once, and cannot be skipped without skipping the merge — and it lands the comment ahead of the only reader it exists for, which is all the PR-open boundary was ever buying. **Moving the write to the implementer is named and rejected**, because the implementer already owes its hand-back a line naming where each grant it acted on was written down, and that line is the **detector** that the write is missing — which is exactly how the third grant surfaced. Move the write there and the detector becomes the thing it was detecting.
- **The two halves of a grant are now stated adjacently wherever the grant is described, because they fail with opposite signatures.** The on-scope half fails *loudly* — forgetting it raises a false divergence alarm on the very next tick — which is why it always gets done; the PR half fails *silently*, at a reader who never learns there was a grant. A reader meeting only the loud half learns the wrong lesson about which half matters. That bar is what caught the seat this release nearly shipped without: `docs/hard-rules.md` still carried the old either/or — the grant written "on that slice's PR **or** on the issue behind it" — which is satisfied by writing only to the issue, and is exactly what the five grants did.
- **A filed follow-up was not required to name a file, symbol or route, so the instrument that re-tests its verdict could be handed input it cannot read.** Checklist item 8 intersects the files, symbols and routes an issue names against the `Owns` of the work about to be dispatched — the fact that moves a verdict re-asking a reader never will. No seat that PRODUCES a filing asked for one. **The instrument is unchanged and deliberately so**: an issue naming no coordinate is still itself the finding, its reason unfalsifiable by construction. What changes is that the filer is finally told, so the finding stops firing on a party that was never asked. **The comment-onto-an-existing-issue case is called out explicitly at every seat**, since a comment records only what is *new* about an observation and is the narrowest input that intersection is ever handed.
- **`/pipeline:write-issue` turned out to be two seats wearing one name, and only one of them was covered.** Its Step 3 *Surface* field already requires the real modules and files plus the consumers each change ripples into, so a full issue written through that pass satisfied the requirement before this release and is left alone. But that pass has a second output which never reaches Step 3: where an open issue already describes the failure, it writes a **comment** instead, and the sentence enumerating what that comment must carry named no coordinate. That is the narrowest input the instrument ever gets, reached by the path where it was least stated, so the enumeration now carries it. **The leave-alone was verified against the right question and the wrong scope** — whether the field carries a coordinate, rather than whether a filing has a second route to the tracker.
- **The close-out's upstream pipeline finding is the boundary the coordinate must not cross.** That one is genericised as a leak guard and keeps the failure's shape while dropping every path, symbol and route. The two filings are now told apart at both seats a reader meets them — in the skill, on the close-out bullet itself, and in the docs, where the follow-up rule is scoped to the arc's own tracker so it stops colliding with what the upstream page says a finding may contain.
- **A reason recorded before the falsifiable-reason bar had no stated disposition, so an arc adopting the bar mid-flight would flag its whole backlog at once.** Every reason written before the bar fails it by construction, so the first cycle after the upgrade would report a wall of findings saying only *this predates the rule* — true of all of them and distinguishing none, which is the uninformative-green failure running in the other direction. **A pre-bar reason is now not a finding on sight: it is re-recorded to the bar at its next re-verdict**, and since item 8 re-verdicts every item filed out of a live arc on every cycle, that population drains rather than accumulating. **No counter was added and the cycle's two numbers stay two** — the migration is bounded rather than tracked. A reason that cannot be written to the bar at all is still the finding, which is the case the bar was set for. **The same disposition now covers the coordinate requirement this release introduces**, which would otherwise be a mid-arc rule with no path for what it found already filed — the exact asymmetry the bullet above argues against: an item filed before a coordinate was asked for gets one added from the tree at its next re-verdict, a repair the loop can make with the tree open, and the instrument's finding stands un-softened.
- **The cycle's artifact had two names, and something had started reading off it.** *The cycle's record* and *the cycle's report* named one surface, harmless while nothing depended on it — but the last release put two numbers in that artifact and made a rule depend on them, so an agent had to decide whether a *record* it is told to write and a *report* it is told to say something in are the same thing. They are. A sweep of `skills/orchestrate/` by concept rather than by the two remembered phrases found four naming sites, three of them already *record*; the single *report* is now *record* too. The words left alone are the ones that mean something else — the *run's report*, the *dispatch report*, the hand-back a checklist item reads, and `"recorded"` scare-quoted as a rejected disposition.
- **Written at every seat that makes a grant, reads one, produces a filing or states the bar.** For the **grant**: `skills/execute/references/dispatching.md` carries both halves on the *take it* answer; `skills/execute/references/reviewing.md` carries the review-boundary trigger, beside the attribution rule it shares a signature with — an artifact outside the diff that only the reviewer can catch; `skills/execute/references/implementer.md` tightens the surface its hand-back audits, so that detector fires on a grant written only to the issue and not merely on one written nowhere; and `docs/hard-rules.md` and `docs/mental-model.md` carry it for the reader who never opens a skill. For the **coordinate**: `implementer.md`, `dispatching.md`'s *file it*, `skills/execute/SKILL.md`'s follow-up rule, `skills/orchestrate/references/reconciling.md`'s *Adjacent* bullet, `skills/write-issue/SKILL.md`'s comment enumeration, and both docs. For the **bar**: the pre-bar disposition sits at item 8, where the re-verdict actually happens, rather than on the bullet writing a first verdict. `skills/orchestrate/SKILL.md` separates the genericised upstream finding from the follow-up's coordinate. **The word "parked" was kept out again** — it names stashed work here, and this entry's own draft had reintroduced it in a second sense, caught by reading the previous release entry against this one. `README.md` was checked against its stated condition and left alone, the question being whether it describes a command, a route between passes, or a helper: it describes none of the three, and its only nearby line is a generic one about what a cycle reports, still true.

## 4.18.0

- **A wrong `Adjacent` verdict was stable under re-application, so the loop's own diligence could not discover it.** Checklist item 8 sends every issue filed out of a live arc back through *Fold vs. file* each cycle — and re-asking a question puts the same item, as the same summary, in front of the same reader, which returns the same answer however wrong it was. One arc ran it as written on the cycle after that text shipped and left seven issues standing `Adjacent`, all seven filed from inside its own slices; the user rejected six of them on sight. The verdicts were not skipped, they were re-performed and came back identical. What finally disqualified three of them was not a re-judgement at all: someone opened the three issue bodies and read the paths, and all three named one function — one already scheduled in the next wave, the other two editing the identical handler and the cache that handler calls. The disqualifying evidence was in the tree the whole time, and the placement test as written is answerable without ever going there.
- **The instrument runs where the coordinates are real, which is at grounding rather than at reconcile.** Item 8 now re-runs each recorded reason exactly as written, intersects the files an item names against the `Owns` of the increment that just merged — answering whether the tree has since moved under it — and compares area alone past the horizon. **The disqualifying intersection is the horizon's, and it runs at step 1**, once `/pipeline:decompose` has ground the next increment and before anything is dispatched into it: the one moment those paths exist, and the one moment the item can still fold rather than ship beside work it collides with. A hit disqualifies *Adjacent* on separability without re-opening the goal question, and folding is the placement test's remaining outcome. Stated only at reconcile it would have run a cycle late, after the colliding slice had already merged — which is the failure it exists to prevent, wearing a checklist's clothes. A green disqualifies nothing and confirms nothing; the recorded claim's re-run is what tests the verdict.
- **Scoping it to the depth that HAS coordinates is what keeps the instrument from being either vacuous or harmful.** Intersecting against every *scheduled* item is the obvious form and is unusable: everything past the horizon sits at shape depth, which carries goal, area and dependency and explicitly no `file:line` and no owned-file list, so an executor either finds nothing to compare and reports an **uninformative green** — this corpus's own name for an instrument with no instances on its target — or grounds the plan early to obtain the paths, rebuilding as a checklist step the one defect `/pipeline:orchestrate` exists to prevent. Past the horizon area is the only shared coordinate, so a match there schedules a re-intersection against the plan item it matched rather than returning a verdict, discharged when that item is ground at step 1. That turns a disqualification the loop would otherwise never reach into one it has scheduled, run at the moment the evidence exists rather than at the moment the question is asked. And an issue naming no file, symbol or route at all is itself the finding, its reason unfalsifiable by construction.
- **The `Adjacent` verdict's recorded reason is now a falsifiable claim about the tree.** The seven reasons on record were *not scope routing*, *island plumbing rather than scope*, *a dead endpoint*, *pre-existing, surfaced from behind a fence* — not one of them a claim a later cycle could check and find false, and all of them re-reading as reasonable forever. The bar is the shape this corpus already holds a negative to: the claim names what it was measured against and the instrument that measured it, the way a verify bar's negative names its baseline and a grounding claim names what established it. *No file this issue names appears in any horizon slice's `Owns`, by intersection at `<sha>`* decays visibly; *island plumbing rather than scope* never will. The one-line rule under all of it — **a verdict recorded without a falsifiable reason cannot be re-examined, only re-performed** — is stated where the reason is written, so a reason that cannot be put in that shape is itself the finding.
- **The standing set was only ever visible in aggregate, and nothing reported its size.** One *Adjacent* a cycle looks like discipline; seven across four cycles is a pattern, and the loop never printed the number. The checklist's *Scope drift* binds the aggregate but reads the remaining plan, and an item filed out of the arc is by construction not in it. Each cycle's record now carries two numbers rather than one, written once that cycle's dispositions are made: how many of the arc's filed items are still *Adjacent*, and how many **cycles** the oldest of them has been. A count alone reads as steady state where an age reads as accumulation. Neither number is the tracker's to report — it carries no *Adjacent* marker and no notion of a cycle — so the count comes off the loop's own recorded verdicts and the age off the increment comments the umbrella has carried since that issue was filed.
- **Written at every seat that states the verdict, runs the instrument, or reads its record.** `skills/orchestrate/references/reconciling.md` carries item 8's three moves, the falsifiable-reason bar and its default instrument on the *Adjacent* bullet, the two numbers on the cycle's record, and the increment comment that gives those numbers a durable home; `skills/orchestrate/SKILL.md` gains the disqualifying intersection at step 1 and, on the decide-don't-ask bar, the line that makes its own *applied once per cycle, so setting it slightly too low multiplies* countable rather than felt; `docs/mental-model.md` and `docs/hard-rules.md` carry the rule for the reader who never opens a skill; and `README.md` gains the clause saying a cycle reports what it left standing as well as what it decided. **The word "parked" was kept out of the shipped prose deliberately** — it already names work stashed under `refs/pipeline-stash` here, and a second sense of one word is exactly what the checklist's own *Vocabulary drift* item exists to catch.

## 4.17.0

- **A fence was a ceiling on what a slice may EDIT and a wall on what it may RAISE, and the second was never the point.** An implementer that found something wrong outside its `Do NOT touch` had three moves — fix it where the fence granted it, file it, or hand back — and all three arrive after the fact, because the channel they run through is the hand-back: by then the agent is dead, its grounding gone and its tree about to be torn down. In the arc that shipped 4.16.0 that cost a release. `README.md` was fenced out of a slice, the change falsified a paragraph in it, this repo ships docs in the same PR as the behaviour, and the implementer's only legal move was to file the fix as a separate issue — out of the very PR that broke it. It was reopened, unfenced, and landed by a second agent in the same worktree. The fence was wrong, nothing could say so while it mattered, and correcting it cost a whole extra dispatch.
- **A fourth move: ask the dispatcher while both are alive.** The dispatcher is the only seat that knows what the siblings own and whether a file is about to be rewritten by another slice — which is exactly why the implementer was fenced out of it — so it answers one of four ways: **take it** (the fence widened for that named path), **the sibling owns it** (the correction carried there instead), **file it**, or **stop, I am re-cutting**.
- **Asking is the DEFAULT for anything outside the fence, and the test is not importance** — it is *would I otherwise have filed this, or sat on it?* An ask is one message and a queue receipt: no turn ended, no worktree, no PR, no gate. A filing is a whole unit of work carrying its own tree, PR and gate, and it moves the reasoning out of the run that still has the files open and onto a person who does not. Two costs that far apart cannot have the second as the default, so **filing became a verdict the dispatcher returns rather than a disposition the implementer reaches for**. The guard against noise is not a high bar but **asking once, with the reasoning and a recommendation rather than a fork** — the shape every other escalation in this corpus already takes. The dispatcher absorbs the question because absorbing it is the job, and the bar for reaching the *user* is untouched.
- **A question is not a stop.** The channel is fire-and-forget — the sender gets a receipt, never a reply, delivered on the dispatcher's next turn — so the implementer carries on with everything the answer does not gate and hands back with the question only if it runs out of independent work first. Today's behaviour survives as the fallback rather than as the first resort.
- **The existing ban on a mid-flight widen is answered rather than deleted, and an implementer still may not widen its own fence.** That ban rested on two reasons: a fence moved by message has no artifact anyone can review, and a live slice that grows is indistinguishable from one diverging. The first is answered by writing the grant onto the slice's PR as a comment that supersedes the brief; the second by the ask being the implementer's and the grant the dispatcher's, so the party that polls for divergence is the party that authorized the growth — and it carries that path into the next tick's on-scope set rather than alarming on work it granted. A widening that arrives unasked, with nothing written down, is still reported rather than taken.
- **The brief NAMES the address, because a sub-agent cannot look one up.** The tool that enumerates live agents is not exposed to a sub-agent, so an implementer reaches its dispatcher only if the brief says how — the same shape as the existing rule that a brief names the PR base branch rather than leaving it to be worked out. `skills/execute/references/platforms.md` gains one row for reaching the dispatching session from inside a sub-agent, established by probe on Claude Code and left as *not established — read your tool list* on Codex, with the rider that what comes back is a receipt and that the listing row runs one way only.
- **The grant's durability step was skipped while this very change was being written, which is why the implementer now audits it.** Two fences moved mid-run during this slice; the first grant was written onto the issue, the second was sent as a bare message by the same party one grant later. A message feels sufficient at the moment it is sent — both parties know what was agreed — and the artifact only ever matters later, to somebody who was not there and is reading a diff in which a fenced file moved for no visible reason. So the duty is no longer the dispatcher's alone: an implementer records in its hand-back where each grant it acted on was written down, and says plainly when nothing was, since its report is then the only surviving record that the fence moved at all.
- **`divergence` is now a definition rather than four implied ones.** The term is what the dispatcher's tick exists to detect and what its destructive lever fires on, and roughly twenty uses across four files each assumed a shared meaning nobody had written down. `skills/glossary/vocabulary/divergence.md` states it: an **unauthorized** scope change, and therefore a **verdict the dispatcher renders rather than a property of the diff** — two agents can produce byte-identical hunks with one diverging and the other not, because what separates them lives in the granting party's record and never in the tree. The exception list shrinks into the definition: a granted widening is not an exception to divergence but not divergence at all, an agent asking for ground is not diverging while it asks since asking is how ground becomes authorized, and a compile-driven ripple never moved the scope to begin with.
- **Written at every seat, since one rule with several readers is one change however many files it takes.** `skills/execute/references/implementer.md` carries the asking side, with the fence-filing rule, the blocker test, the out-of-scope warning sweep, the enumeration floor and the brief-contradicts-itself case all re-routed through the ask; `skills/execute/references/dispatching.md` carries the deciding side and the address the brief must name; `skills/decompose/references/slicing.md` writes the default into the fence field itself; `skills/execute/SKILL.md`'s follow-up rule sends an out-of-fence item to the dispatcher before the tracker; and `docs/hard-rules.md` and `docs/mental-model.md` carry it for the reader who never opens a skill — including the slice-depth row, whose ceiling is no longer immovable for the life of the run. `skills/orchestrate/SKILL.md` and `skills/orchestrate/references/reconciling.md` each carried a justification clause asserting the opposite as its stated reason, and all three narrow to *a widening nobody granted* while the rules they justify stand unchanged; the loop's per-tick list gains the step that answers a queued ask, since a channel nobody is told to listen on is not a channel and its absence goes green at every other step. `skills/review/SKILL.md` was the seat most easily missed and the one that needed it most: its *Flagged, out of scope* bucket is admitted on the fence by definition, and it handed that pile back to the implementer labelled *file* — the disposition this change demotes — so what is genuinely left there is now the caller's to RAISE.

## 4.16.0

- **The placement test asked whether the arc could still compile, pass or ship without an item, and an arc's goal is routinely none of those three.** An arc filed so that a class of defect stops happening builds, passes and ships perfectly well without the check that stops it recurring — so the one item its goal was not true without read as *Adjacent*, got filed out, and the arc closed green with the work it was filed for still outstanding behind it. The verdict is now the goal's — *is the arc's stated GOAL true without it?* — with compiling, passing and shipping named as the **floor** of that question rather than the bar, read in the words the issue states its goal in. **A milestone, a release line, a shared label or a sibling issue is none of it a substitute**: each puts the work beside the arc without making the arc's goal true, so *out of the arc but shipping alongside it* is the *Adjacent* verdict wearing a softer word, and takes that verdict's narrow bar.
- **`Adjacent`'s size condition was reachable before its goal condition, and the one that gates is the goal.** The triage step above the placement test offered *genuinely an arc in its own right* as a filing in its own right, so a heavy forced item became a filing before the goal test was ever asked; that step now defers to the test rather than pre-empting it. Where the two conditions are defined they are ordered explicitly as well — weigh an item for size only once the goal is met without it — on the principle that **size answers how work is CARRIED, never whether it is IN**. Forced-and-large is therefore not a verdict at all but a plan defect, the arc planned smaller than its goal, reported as one with the re-plan recommended rather than filed away. And a mixed finding is split at the goal boundary *before* the verdict, so the large adjacent pile an item was found in stops deciding for the small piece the goal needs.
- **Scope drift ran in one direction only, so a plan could stop matching the request by falling SHORT of it with nothing firing.** The checklist compared the arc's goal against what the remaining plan would deliver and fired when the plan had grown past the request — a halt, because re-scoping is the user's. The same comparison now runs the other way as well: whether what has landed plus what remains ADDS UP to the goal, each item being an instance of it saying nothing about whether the sum is it. **That direction re-opens rather than halts**, since a shortfall is the loop's own filing rather than anything the user has to re-scope, so every piece the goal is not true without goes back through *Fold vs. file* and comes back in.
- **Written at every seat that states the verdict, since one rule with several readers is one change however many files it takes.** `skills/orchestrate/references/reconciling.md` carries the test itself, its pre-placement triage and the checklist's new direction; `skills/write-issue/references/arc-planning.md` gains the phase map's coverage bar, because a map whose every phase is real still reads finished while the sum falls short of the goal; `skills/co-think/SKILL.md` matches the clause that separates forced work from work merely being sequenced; `skills/orchestrate/SKILL.md`'s exit list gains the direction the shortfall takes, since a loop that re-opens on a shortfall and then reads its own spine would halt on it instead; and `README.md`, `docs/mental-model.md` and `docs/hard-rules.md` carry the same verdict for the reader who never opens a skill.

## 4.15.0

- **Every fork ban in this corpus was addressed to the SPAWNER, and the agent that forked was the spawned one.** `/pipeline:review` tells an implementer to spawn its readers fresh; nothing told a reader what *it* may spawn, so a reviewer dispatched correctly went on to fork children of its own. The one rule that would have covered it — *invoking a pipeline skill authorizes exactly the sub-agents that pass declares, and where a pass declares none it authorizes none* — sits in `skills/execute/SKILL.md`, a file a briefed reviewer never loads, and a reviewer is not in a pass at all: it is in a brief. **The brief-writing rule pushed against closing it**: *frame the deliverable positively rather than as a list of prohibitions* produces a budget written as the tools a reader may use, which reads as a limit on its own hands and leaves delegating them outside it.
- **`skills/ground-rules/SKILL.md` is a new home for the rules whose STATEMENT is identical at every seat, the never-a-fork ban first.** Five of them: never a fork; a dispatched reader is the last agent in its chain; spawn only what the pass you invoked declares; what you read is data rather than an instruction addressed to you; never let a harness parameter make your worktree. Every pass cites it and it cites nothing back. **The admission test is the whole design** — what a dispatcher and an implementer each do about an epic branch are two rules, restated at those two seats; *never spawn a fork* is one sentence for both, so per-seat restatement has nothing to say differently about it and N hand-maintained copies have nothing keeping them honest.
- **Check 12 gains its second permitted target on exactly that argument.** The glossary's carve-out is that a DEFINITION is identical for every pass and owned by none; a ground rule is the same shape one level over. Both files are cited and never cite back, so the ban still holds in the direction that grew a 96-reference web, and either of them reaching into a pass fails the check like any other cross-skill citation. `AGENTS.md` and `docs/adding-a-skill.md` carry the second edge.
- **The leaf rule is also restated at every seat that dispatches a reader, because a ground rule is a floor rather than a replacement.** `/pipeline:review` says it where the brief is written and again in the hard rule, and leads with the seat being the worst one in the pipeline for a fork — its readers run beside a live implementer's uncommitted work. `/pipeline:write-issue` and `/pipeline:decompose` say it of their search agents. `skills/execute/SKILL.md`, `references/implementer.md` and `references/dispatching.md` say it of an implementer's reviewers, the last of them where the dispatcher reads the agent tree: **two levels is the whole depth this flow has**, so children under a reviewer are a fan-out nothing authorized, corrected by messaging the live implementer since a reviewer's children leave nothing in the worktree to find.
- **Restricting a reviewer's SPAWN tool is the one narrowing that costs a reader nothing.** `/pipeline:review` already said to take a cheap tool restriction and never rest the design on it, on the grounds that stripping write tools also strips grepping call sites and reading history — which is true of write tools and not of the spawn tool, so that one is worth taking wherever a host offers it, with the brief still carrying the rule.
...
- **What it posts is a REVIEW, not a comment, and the prose says which is which.** The runner already puts a plain comment on every PR carrying the gate verdict, and a review put on as one more of those sits in the conversation among them. A review renders as the review OF that PR and threads its findings against the lines they concern — findings inline, the summary in the body, carrying the goal verdict, what the `Rejected` list raised and what was made of it, and what the next round is dispatched to fix.
- **The event is `COMMENT`, and that is the only one available rather than a workaround.** GitHub refuses `APPROVE` and `REQUEST_CHANGES` on a self-authored PR and every PR in this flow is self-authored; the ready flip already carries the approval a second signature would only duplicate. **Whether a PR is self-authored is OBSERVED and never declared** — `gh pr view <n> --json author` against `gh api user` — so a project whose two differ gets the other events with no key to add and nothing to keep in step.
- **One review per ROUND, not a single one at the end.** A needs-changes review is posted and the loop re-enters as usual, the fix agent going out against a finding now written on the PR as well as carried in its brief — the brief stays the dispatch instrument and the posted review is the record. The satisfied review is the last one and the ready flip follows it, in the helper path and in the hand-run sequence alike, so a PR that took three rounds carries three reviews in order and the last lands against the commit being merged. A single summary at the end would flatten those rounds into one verdict and lose the part worth keeping.
- **It stays outside `bin/`.** The helper CLI contract is frozen and shared by two implementations that neither owns alone, so the post is the dispatcher's own step run before it invokes the helper — no new argument, env var, stdout line or exit code.
- **The attribution ban widens from a pair to a CLASS.** Every seat stating it enumerated two artifacts — commit messages and PR bodies — so a posted review, a third of the same kind published in a public repository, satisfied every one of those enumerations while carrying a trailer. The ban is now stated as the class it always meant: **anything this flow writes to GitHub in the maintainer's name** — a commit message, a PR body, a posted review and its inline comments, an issue or a comment on one. **Both axes are now general**: the named forms are instances and so are the named artifacts, since an enumeration of either is satisfied by every member it omits. Five passages across five files enumerated that pair and all five are rewritten, along with the bullet arguing why the pasted brief block states the rule generally — now general on both axes. **Widening the class created readers that had no seat at all**, so the rule is added at five more: the human-facing hard rules, and the four passes that write to GitHub without ever loading the one that carried the ban — `/pipeline:orchestrate`'s close-out, which files a finding into a public tracker, `/pipeline:write-issue`, `/pipeline:decompose`'s emit step, and `/pipeline:cut-release`, which authors a commit and a PR. Eleven passages across ten files.
- **`isolatedBy: null` no longer has to be written for a resource that is shared but does not CONTEND.** `null` is specified as *shared and stays shared* and read as *narrow the wave*, so a content-addressed cache whose concurrent access is its designed mode had no honest value: writing the truth switched fan-out off for the whole project to protect something nothing can corrupt. **No new value and no new key** — a non-null `isolatedBy` may now name why the resource does not contend as well as a mechanism that isolates it, stated where the key is specified and where its consequence is read, so the honest answer and the correct dispatch decision are the same answer. The onboarding pass's falsification step says what it can be held to there: an entry naming non-contention has no entry point to disable, so the reversal cannot run and the overlap alone is the evidence.
- **`changelog` admits a directory as well as a file.** The key assumed a changelog that accumulates in place, so a project keeping one file per release had nothing honest to write: naming the current version's file points at something that changes identity every release, and omitting the key leaves the release pass halted at a step it has to perform. **No new key** — the admitted form widens to *the one file a section is prepended to, or the directory a per-version file is created in*, and the release pass reads which of the two it was handed.
- **A brief's grounding claim is now held to its instrument the way a verify bar's already was.** Running a command over a file is not reading it: a claim can come from a command that executed cleanly and printed plausible output while nobody opened what it found. The rule now says a command LOCATES a candidate and OPENING what it found is what establishes the claim, and names the two properties the real failures took — **containing a string and acting on it are different facts**, which every `grep -l` or `grep -c` over source cannot separate, and **where two outputs of one command disagree, that IS the finding** rather than noise to pick the agreeable half from. No new gate check: nothing mechanical can read a brief against a tree.
- **The README's `PATH` export named the clone install and said so nowhere.** A reader on the supported marketplace install exported a directory that does not exist, and nothing errors — `export PATH=` succeeds for any string, and the failure surfaces later as `command not found`, which reads as a broken plugin. The snippets now say which install they are for. The marketplace install needs no export inside Claude Code, where `bin/` is already on the shell tool's `PATH`; in a plain terminal it sits under `~/.claude/plugins/cache/<marketplace>/<plugin>/<version>/bin`, which carries the installed **version**, so no hardcoded export stays correct across releases and none is offered.
- **`setup-workspace`'s usage line names `--exclude`.** Both ports have parsed the flag all along and neither usage line mentioned it — a gap the parity check is structurally blind to, because it compares the two lines against each other and they agreed. Added to both, byte-identical apart from the script name. Usage text only: the frozen contract takes no new argument, env var, stdout line or exit code.
- **New gate check 16: every slice-field enumeration names every field.** A field enumerated in N places had nothing keeping the N in sync, and check 12 is what made that drift invisible — a skill may not cite another skill, so each copy is internally consistent and structurally forbidden from pointing at its siblings, which leaves two copies disagreeing unreachable by every other check by design. **The field set is DERIVED, not listed**: the glossary's *grounding depth* entry already declares itself canonical, so the check reads the names out of that entry's own bold run — a hand-written second copy in the checker would be the disease one level up, and a field added to the entry now reds every copy that lacks it. Only the alias spellings and the list of enumerating passages are held in `scripts/check.sh`, which is contributor-only and unshipped. **It is scoped to the enumerating PASSAGE and never the file** — every one of these files says *goal*, *brief* and *verify* in ordinary prose many lines away, and a whole-file search returns green whatever the enumeration says; inside a bullet list or a template a spelling counts only at a declaration position, since *Wave 2 owns those* sits in the emit template. Anchors match whitespace-collapsed text, so re-wrapping a paragraph cannot truncate a scope, and an anchor that stops resolving is a failure rather than a skip. It found two live disagreements on the way in: the slice-depth row in the mental model was short by `Goal`, and `/pipeline:decompose`'s own skill description — the most-read copy in the corpus — was short by `Derives`.

## 4.13.1

- **`/pipeline:cut-release` picked its route on a question every project answers the same way.** The test was *are you standing on the integration branch* — and the main checkout holds the integration branch and nothing else, which two other passes in this plugin state as an invariant, so the answer is yes wherever the config is not already drifting. Every project therefore took route B, *version bump only*, the project that names one branch per release included; route A — cutting the branch, which is the reason the pass exists — was reachable only from the drift state it is meant to repair, where it would cut a second branch on top of the first. **The route is now one question about the branch's NAME: does moving the version make it wrong?** `release/0.4.0` at `0.5.0` stops being true and rolls with the version; `main`, `develop` and `release/0.4.x` are each as true at the new version as at the old, so the version moves in place and nothing is cut.
- **`HEAD` against `integrationBranch` keeps its real job one bullet up, as the DRIFT test — and which side is stale decides the disposition.** The pass's own description names that drift as an entry point and its body had no handling for it, so it fell through to route A. A **declared branch that does not exist** is the config pointed at the next release with nobody having cut it, which is this pass's moment: route A, branch already named. A **declared branch that exists** is a roll made without this pass: report both names and stop, `/pipeline:setup` owning that repair.
- **Route B's confirmation sentence promised route A's work.** Step 1 offered one sentence to confirm and it ended *and point `integrationBranch` at the new branch* — a promise no version-bump-only run keeps, put to the user as the thing they are agreeing to. Each route now has its own sentence.
- **`/pipeline:setup` still said nothing in the pipeline is present when a release branch is cut.** `/pipeline:cut-release` shipped in 4.10.0 and stands exactly there, moving the version, the changelog and the field in one commit; the sentence explaining the drift setup detects had not moved with it, and it reads as a standing property of the flow rather than as what happens when a roll skips that pass. It now names the pass and the two states that produce the drift, which want opposite repairs.

## 4.13.0

- **`/pipeline:review` stops being a stance the author takes over its own diff and becomes a pass that dispatches one briefed reviewer per dimension.** The reader and the author were the same agent, and they were the same agent even where the dispatcher and the implementer are properly separate — six lenses run one after another by the party that wrote the code. The pass now dispatches up to seven readers, weighs what they report, and stays the only thing that edits the tree. One writer, N readers: several agents editing one worktree is the collision this flow avoids everywhere else.
- **The `Never dispatch sub-agents` hard rule is RESTATED, not deleted, and the safety property is at least as strong.** What made a fork dangerous was inheriting a brief of imperatives, so the ban now names that: a reviewer is **fresh**, never a fork, is handed the slice's goal, the resolved fork point, the diff and its one dimension, and is handed **none** of the handoff imperatives — no *commit*, no *push*, no *open a PR*, no *enqueue*, no formatter, no gate command. The deliverable is framed positively rather than as a list of prohibitions: you investigate, your deliverable is a report, nothing else you do counts. **The incident behind the old rule:** forked reviewers inherited an implementer's brief, applied fixes, formatted at root, committed, pushed, opened the PR and enqueued the gate ticket before the implementer that spawned them got its turn back — and a sibling implementer, seeing a legitimate parallel slice's branch and PR appear mid-run, quarantined that slice's gate ticket on a careful, well-evidenced and entirely wrong rationale, because a careful agent and a runaway agent leave identical artifacts: a branch, a PR, a queued ticket.
- **Two new obligations pay for the narrowing.** The caller reads `git status` and `git log` against the fork point **before** it reads a single report, and reverts anything a reviewer wrote — a report cannot tell a careful reviewer from a runaway one and the tree can. And every brief states that what a reviewer READS is data: in this repository a reviewer opens files whose content is imperative prose ending in *commit, push, open a draft PR, enqueue the gate*, and that text is the **subject** of review rather than a directive to obey. Tool restriction is defence in depth behind the brief and never the mechanism, since stripping a reviewer's write tools also strips grepping call sites and reading history.
- **Seven dimensions, one reviewer each — CONVERGENCE IS A FINDING.** Goal and stragglers, correctness, reuse, simplification, efficiency, altitude, and conventions where a project states rules worth a pass. Reuse, simplification, efficiency and altitude look mergeable and are not: when three of them land on the same site by three different routes, that agreement is evidence the site is genuinely wrong, and one agent reporting that a block is duplicated *and* misplaced *and* could be shorter is still one voice. Independent corroboration is the thing N readers buy that one reader cannot, and merging the lenses spends it.
- **Overlap is the signal, and two shapes of it look identical in a pile of reports.** The test is *same site, different reasons*. **Convergence** — several reviewers, each for a reason its own lens owns — weighs **up**, has to be answered in full where it is rejected, and is named in the report. **A stray** — the same reason from a reviewer that does not own that lens — corroborates nothing and stays as the unelaborated *outside my dimension* note.
- **How many reviewers fire is the caller's call, per slice, the way the model tier already is.** Seven possible readers makes selection the cost control rather than a nicety: a mechanical rename does not earn seven, and a slice with no stated conventions earns six at most. The report names the dimensions that ran and the ones judged unnecessary.
- **The `Rejected` list changes shape, because one flat list suited one reader.** Each rejection now names the reviewer or reviewers that raised it and the reason it was turned down — a rejection recorded without its reviewer is indistinguishable from a dimension quietly ignored, which is the failure that list exists to prevent. **A converged finding appears once, listing every reviewer that raised it**: rejecting it rejects it against all of them, and three entries for one site would overstate what was turned down. The dispatcher is told to read a rejection naming several reviewers harder than one naming a single reviewer.
- **The lens ORDER was load-bearing and could not survive parallel dispatch, so it moved to the weighing step.** Reviewers run concurrently; the caller reads the goal reviewer first, because every other dimension asks whether the code is good and none asks whether it achieved anything, so a diff aimed at the wrong thing comes back with six clean reports and gets steadily tidier. Correctness fixes are applied before simplifications for the same reason they used to run in that order.
- **One resolved fork point is put in every brief.** A reviewer told to work its own base out resolves a different one — its `HEAD` is the caller's, and nothing else about its starting position is — and reports written against different bases cannot be compared, which is the one thing this pass does with them.
- **Four callers restate the rule and all four moved with it.** `skills/execute/references/implementer.md` bans a pass that forks its reviewers *and* one that hands a reviewer the implementer's brief; `skills/execute/references/dispatching.md` carries the corrected tell — an implementer waiting on review sub-agents ran the pass as designed, and the worktree's `git log` is what distinguishes a runaway from a reader; the pasted review-slice block no longer says *inline, spawning nothing*; `skills/execute/SKILL.md` gains the fresh-not-a-fork rule on the implementer's own review step, where the dispatcher already had one. A concept sweep — for the rule, not for the phrase *spawns nothing* — found these four plus the pass table's row in `README.md`, which described it as run inline and now names the dispatch.

## 4.12.0

- **The front page orients; every reference section is its own page under `docs/`.** `README.md` was 12,369 words (`wc -w`) across eleven `##` sections — more than twice the largest file the plugin ships (`skills/execute/references/dispatching.md`, 5,856 words) and the only substantial doc no gate measured, since check 10's per-file and per-sub-skill ceilings read `skills/` alone. Seven of those sections are now seven pages totalling 10,997 words; the front page is 2,695 words of orientation plus a table pointing at them. **`.agents/worktree.json` carries a `docsPaths` entry per page** — 9, up from 2 — so *which doc did this change make stale* stays a judgement against a stated condition rather than a guess at a directory.
- **A key a project must declare is documented where a person reads, not only where an agent does.** `examples/worktree.json` declares 18 keys and the README documented 15: `bumpFiles`, `changelog` and `docsPaths` had zero mentions in it while the skills carried all three, so every agent could read them and nobody setting a project up could. All 18 are on the config page now.
- **The frozen helper contract's environment surface is documented in full.** It names five variables and only `WORKTREE_HOME` had ever appeared in the README; `REPO`, `WORKSPACE`, `WORKTREE_DEST` and `MERGE_PR_FORCE` had zero mentions. `REPO=` is the one a reader most needs: every helper resolves its target repo from your cwd, so a `merge-pr.sh <n>` run from a *different* repo's checkout operates on that repo with every signal reading clean, and naming the repo is the override for it.
- **Real merge commits are the default everywhere, `"epicMerge": "squash"` is a project-declared alternative at exactly one boundary, and `Never rebase` is its own rule with no exception.** The rule was stated twice in shapes that disagreed — a prohibition-with-an-exception whose closing sentence then retracted the exception from rebase, a sentence that existed only to repair the framing. Stated as a default plus a declared alternative, neither half needs a retraction, and the boundary is named once: an epic branch collapsing back into the integration branch it was cut from.
- **Declaring `integrationBranch` is what makes that squash REACHABLE, not what blocks it.** The old wording had it backwards. Declared, `merge-pr`'s boundary test is one question about the **head** branch — is it the integration branch? — and an epic close-out's head is not, so it qualifies even where the integration branch is also the repository's default branch. The default-branch comparison is the fallback for a project that declares nothing, and it is that fallback which declines the genuine epic boundary wherever work lands on the default branch. `scripts/port-cases/squash-boundary.tsv` settles it in both ports. **This repo now declares `"epicMerge": "squash"`.**
- **Check 11 reaches the prose that states its own rule.** It listed `skills/*.md` only — 30 tracked files — so `AGENTS.md`, the file that promulgates the ban, was the one place it did not reach, and it carried 5 live instances of a banned phrase with the gate green. Scope is now every tracked `*.md` but `CHANGELOG.md`: 39 files, adding `AGENTS.md`, `README.md` and the seven new pages, which is the direction prose extracted out of `skills/` now leaves by. **`CHANGELOG.md` is exempt by the rule's own logic** — the ban does not delete an incident, it relocates it to the change that fixed it, and a release entry is one of the two places it points to.
- **The ban is a PROPERTY and the pattern is a named BACKSTOP, so no phrasing was added to it.** The property: name the failure a rule prevents in a clause that shares its sentence with the action, never in a sentence or a paragraph of its own. An enumerated ban is satisfied by every form it omits, so a green says those phrasings are absent and never that the property holds — one comma inside a matched phrase already puts a war story past the regex, and a longer list only makes the green look better. Adjudicate against the property; where you cannot tell, leave it out. A by-concept sweep then found across 4 files what the pattern could not, the largest being two paragraphs in `skills/review/SKILL.md` sitting one comma from a phrase it holds.
- **Check 8 resolves a markdown link the way a renderer does — from the directory of the CITING file.** It resolved every cited path from the repository root, which cannot tell a correct `../skills/<x>.md` from a `skills/<x>.md` written inside `docs/`: both exist from the root, only one renders, so a link moved into a subdirectory can render dead with the gate green. Any cross-directory relative link that is not a `skills/*.md` path was invisible outright. A second half now resolves 41 in-repo link targets from their citing file, while the bare `skills/…` prose citation keeps its root resolution — 238 of those resolve across 29 distinct paths on this release's tree — because the two forms have different readers. Link syntax inside a code span or a fenced block is skipped for the same reason no renderer resolves it; external URLs and `#fragments` stay out of scope, the second because checking it means reimplementing every renderer's slug rules.

## 4.11.0

- **The drain rider named no mechanism, so the dispatcher filled the blank — and the two available forms differ on exactly the thing the rider is about.** *Draining the gate queue* said to bound a big queue and background it, full stop. Reached through the harness's tracked background flag, that manufactures a completion event for the one pass the same bullet says carries no signal, and a dispatcher handed that event has a verdict-shaped arrival about tickets it does not own. The rider now names the **detached** form — `nohup … &`, or whatever the host's shell spells detachment as — and says why the tracked flag is wrong: not that it is heavier, but that it would create an event.
- **A named mechanism reads as a contradiction of the check-backgrounding ban unless the disambiguation is written down.** *The foreground-handoff rule* and the implementer's *Never background a check and end your turn on it* forbid ending a turn on a backgrounded check's **result**; the drain rider ends its turn on the tick, on no result at all. Left implicit, a dispatcher resolves the clash by foregrounding the drain — the serialized wait the `--max N` bound exists to avoid — or by reading the ban as dead.
- **The ledger watch could be described as present and not detected as MISSING.** *Wait on your own tickets settling* set the instrument out in full and said nothing a dispatcher without one could notice about itself. The section now carries the negative: learning a verdict from a timer, or from a drain's exit, is what an unarmed watch looks like from the inside, and both routes read as diligence. **This is the defect underneath the incident that produced the release** — a dispatcher filled the backgrounding blank with `nohup … &`, got no completion notification, fell back to polling on its own timer, and concluded the fix was the tracked background flag, which is backwards. It believed a timer was the only route to a verdict because it had no ledger watch armed.
- **`Monitor` was a bare tool name in a file that does not name tools.** `platforms.md` declares itself the only place a host's tools are named, and the persistent-watch capability had no row — so a Codex dispatcher met a tool it could not resolve, with nothing telling it to read its own tool list instead. The table maps it now, with the unestablished cell spelled the way the stop lever's already is. **No row for backgrounding a shell command**: `nohup … &` is portable shell rather than a host tool, and the drain rider carries it.
...
- **Three more facts a project owns, declared rather than searched for.** `bumpFiles` names every file whose version string moves; `changelog` names the one file a new section is prepended to; `docsPaths` gives `{path, when}` per doc tree. Each is a claim of **completeness** — searching finds some and nothing tells a searcher it found them all, which is why this repo's two host manifests can disagree and ship a different release to each host.
- **`bumpFiles` answers WHICH, never WHETHER**, and `changelog` is separate from it because prepending a section is a different operation from replacing a string — handed one flat list, an agent looks for a version field to swap in the changelog and either overwrites the last release's heading or reports success having done nothing. Omit both where nothing hand-edits a version: tag- or commit-derived versioning has tooling that owns it.
- **`docsPaths` completes a rule that already shipped.** An implementer is told to update the docs its change made stale, and told its own search will come back empty because user-facing prose carries none of its new identifiers — and was never told where to look instead. The `when` also makes the per-doc verdict a judgement against a stated condition rather than a sentence composed on the spot.
- **A config naming a branch the main checkout is not standing on now stops the arc.** That checkout holds the integration branch and nothing else, so the two disagreeing means the project rolled its branch and the config did not follow. It is a stop rather than a note because the file is read for **provisioning**: knowing the right branch does not fix a stale `install` or stale env symlinks, and the whole config freezes at the first worktree.
- **A ninth pass, `/pipeline:cut-release`, owns the one moment nothing was present for.** Cutting a release branch happens outside the arc entirely, so `integrationBranch` went stale, the version moved invisibly, and the flow could only detect the damage afterwards. The pass observes where the main checkout is standing, confirms in one sentence, then cuts the new branch in a worktree and makes the bump, the changelog section and the config move **one commit** — the field that stops those facts drifting cannot land separately from them.
- **It declares no config of its own.** An earlier draft added a `trunk` key for the branch releases are cut from; the pass is interactive, so it looks and asks instead. Config is for facts a pass cannot see, and where a person is standing is not one of them.
- **Two families, said out loud.** Six **arc** passes ship work into the integration branch; two **project** passes — `setup` and `cut-release` — change the project itself, run beside the arc, work in their own worktree, and terminate at a reviewable change they do not merge. `cut-release` inherits setup's posture rather than inventing one, including refusing to run while any worktree is live, since the config it rewrites is frozen once trees exist.
- **Scoped to the repository side and nothing past it** — no tags, no publishing, no deploys, no CI triggers. Those are per-project, and a pass reaching for them invents policy it cannot know.
- **`/pipeline:review` asks what a slice leaves BEHIND, not only whether it met its goal.** Goal-met asks whether the diff reaches the goal; nothing asked what it stranded on the way — a fourth call site left unmigrated, a helper the change made dead, a doc the behaviour just falsified. The reviewer is the only reader who sees the whole diff while it is still uncommitted, so a straggler caught there is one edit and caught later is its own task. **A straggler neither fixed nor reported is the one shape of finding that reaches nobody**, since the diff reads as complete to everyone downstream.
- **The reconcile checklist reads the hand-backs, which it never did.** Its nine items all interrogated artifacts that outlive the run — the plan, the tree, the request, the tracker, the seam map. An implementer's context does not outlive it, and the hand-back is the whole of what survives. Above all it carries **the review pass's `Rejected` list — by construction a list of things somebody noticed and chose not to do** — which was routed up to the dispatcher deliberately and consumed nowhere. The tracker item could never catch it, because a rejected finding was never filed.
- **Fold-vs-file gains the cost that actually decides it.** A filed item becomes its own task with its own worktree, PR and full gate: marginal inside the arc, a whole unit of work outside it. And **folding is not absorbing without scrutiny — it is routing into scrutiny**, since folded work is ground at the next horizon by a pass whose job is to validate and escalate, while filing removes it from that entirely. So fold is the default, and *Adjacent* is the narrow verdict rather than the convenient one.
- **Filing now surfaces to the user; folding does not.** The decide-don't-ask bar treated fold-vs-file as wholly the loop's call. Filing is the one disposition that leaves the loop's hands, so it goes to the user with the reasoning and a recommendation, at the same bar as any other escalation.
- **The two ports are now compared by the gate, not by hand.** The parity check reads surface — a sibling exists, the usage lines match, the `.ps1` is ASCII — and makes no claim about behaviour, so a predicate written twice could diverge and stay green, surfacing as the same PR merging differently depending on which shell the platform handed the user. `scripts/port-cases/squash-boundary.tsv` is one table asked of both implementations. **A renamed predicate fails rather than skips**, since a silent rename is how the pair would stop being compared while the check kept reporting ok; where `pwsh` is absent the bash half still runs and the other half says so.
