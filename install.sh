#!/usr/bin/env bash
# Install the decompose skill on this machine.
#
#   ./install.sh
#
# Symlinks the skill into every agent skills home Claude Code looks at:
#   skills/decompose/  -> ~/.claude/skills/decompose/  AND  ~/.agents/skills/decompose/
#
...
#
# decompose is a PLANNING skill — it ships only the skill, no helper scripts or
# per-project config. On the GitHub path it reads the same per-project config the
# orchestrate skill owns (~/.worktrees/config/<project>.sh); install that via the
# worktree-orchestrate repo if you want gate / brief-convention awareness. With no
...
echo "Installing decompose ($MODE) from $REPO"

# The decompose skill — into every skill home
for home in "${SKILL_HOMES[@]}"; do
  link "$REPO/skills/decompose" "$home/decompose"
done

echo
echo "Done. Sanity check:"
for home in "${SKILL_HOMES[@]}"; do
  echo "  ls -la $home/decompose"
done
echo
echo "Next: open Claude Code in your repo and try  /decompose  — see README.md."
