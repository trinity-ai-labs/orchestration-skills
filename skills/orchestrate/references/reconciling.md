...
**Two costs sit under this and the guard names both**: an arc that absorbs everything never terminates, and **filing moves the reasoning from the run with the tree open to a human without it**, a channel the close-out's pipeline-finding question also feeds. **When they conflict, lean toward settling**, since non-termination is visible from inside the loop and backlog transfer is not; **settling is not absorbing**, producing a *decision* where folding produces a *slice*. **Whatever you file or ask carries the reasoning and a recommendation, not a fork.**
...
- **"Filed" means filed — and *linked* is one act or two.** **The relation decides which: `Part of #<umbrella>` is containment and takes the body backlink AND the native `sub_issues` POST, while a bare `Follows #<N>` on a plain issue is provenance and takes the backlink alone.** `skills/execute/SKILL.md`'s follow-up-ownership rule binds this loop as it binds an implementer — **a bullet in a report is not a follow-up** — and **the two-link case is this loop's default**; the call and its database-id trap are in `skills/execute/references/implementer.md`'s *File your follow-ups BEFORE you hand back*.
...
**The body carries one more piece of arc state, and the loop is the only pass positioned to hold it: the arc's contract-seam map, a running union rather than a per-cycle re-derivation.** Seed it from the issue body's `Seams` field, grow it with each cycle's decomposition, keep it in the **body** beside the plan, never assembled from the comment thread; a closed seam leaves it and *The seam map* records why (`skills/decompose/SKILL.md`'s *Contract seams* defines it).

Read the issue with `gh issue view <N> --comments`; write the rewritten body and each increment comment through the `gh api` REST endpoints, passing a body read from a file with `--field`, never `--raw-field` (`skills/decompose/SKILL.md`'s *GitHub write mechanics*) — only `--field` expands a leading `@` into the file's contents, and the raw form stores the path and **exits 0 with a comment URL**.
