...
**An epic branch's prefix carries no mechanical meaning; its leaf does** (`skills/concepts/references/branch-leaf.md`), so that slot takes the real branch name. **It should not read like a slice branch**, or it is indistinguishable in a PR list from the `feat/<leaf>` slices merging into it. **And its leaf must be one no slice branch you name reuses** — the likeliest collider is the closing docs slice. Check it every cycle against the slices you are grounding now.
...
- **Write the body to a file and reference it with `-F` (not `-f`)** — `skills/concepts/references/gh-api-file-body.md` says why, and why the wrong one exits 0. **Verify after**: refetch the body and confirm it's the markdown, not the literal path.
...
- **Native sub-issue link:** `skills/concepts/references/sub-issue-link.md` carries the call, and the two ways of getting its id wrong that both read as a missing endpoint.
- **Cross-reference, don't auto-close.** Each sub carries `Part of #<umbrella>` in its body and the umbrella carries that sub in its `- [ ] #<sub>` checklist — both directions, every time — the backlink is a rule rather than a formatting nicety, because only it is readable from the child's own body, which is all an agent arriving there directly has. The dispatcher then closes each sub by hand as its PR merges, never a `Closes` keyword (`skills/concepts/references/closing-keyword.md`): the hand-close is correct under both readings, a no-op when GitHub already did it.
