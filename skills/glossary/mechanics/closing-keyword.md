...
**The predicate is the PR's base against the repository's DEFAULT branch, and nothing else.** Another
branch stands in for it only where the two genuinely differ. Where a project's integration branch
(`skills/glossary/vocabulary/integration-branch.md`) simply *is* the default branch, there is nothing
between a PR and that branch, so the keyword is **live** there.
