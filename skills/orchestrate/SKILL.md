...
on the integration branch at step 2. **Where one WAS cut, green takes a third thing: that close-out PR's own
full `/pipeline:review` panel, run against it between its open and its gate** — a close-out that gated and
skipped the panel is not yet green, since a gate says the suite passed over the combined tree and cannot say
whether the slices COMPOSE, and that combined diff is the first and only place they sit together. **Apply
nothing it raises yourself** — you write no code at any step of this loop — **dispatch a fix agent into the
epic branch's own worktree, which pushes onto that same close-out PR**, then gate. **A close-out that cut no
epic branch is untouched by this**: the checkpoint exists only where a branch was, each of those slices having
already been reviewed as its own PR. **A finding the umbrella body still carries that no checklist line,
