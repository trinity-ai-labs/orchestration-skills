...
**And one value you must ASK for, because it is the only one in the file no file in the repo can tell you: what does the gate touch that lives OUTSIDE the worktree?** A lockfile names dependencies, `package.json` names scripts, CI names the gate — none of them names the Postgres instance the suite migrates, the Redis the queue tests flush, the cache directory two runs share, or the fixed port an e2e run binds. Ask it in those words, then ask the follow-up that turns the answer into something testable: **does each worktree already get its own, and what makes that happen?** Write both into `sharedResources` — the resource, and the mechanism plus the entry point it sits at — and write `[]` when the honest answer is that the checks touch nothing outside their own tree. *The failure this prevents: this is the one key that cannot be ground, so a pass that only reads files produces a config where every other value traces to something and this one is silently absent — and absence here is not the safe reading (`skills/execute/SKILL.md` → *Per-project config*). Nothing errors, because the failure is a repo whose worktrees are declared parallel and are not.*
...
6. **Tear down** the verification worktrees with `remove-worktree.sh` (or `remove-worktree.ps1`) — both of them, if item 5 cut a second. **And say plainly what the teardown does not reach**: anything a per-worktree resource mechanism created is still there, named after a path that no longer exists, and nothing in this plugin drops it. That gap is unowned today; report it rather than closing it here.
...
**That exception is about the FIRST config. A later CHANGE to one is a different case, and it is not a second exception.** This pass is the likeliest producer of such a slice, because it writes the file every later `envFiles` entry, `install` change or new key is a change *to*. Such a slice gets a perfectly good worktree and still cannot exercise its own change: `setup-worktree` reads the **main checkout's working copy** of `.agents/worktree.json`, so every worktree cut anywhere until the change merges is provisioned from the pre-change file, and neither pushing the branch nor cutting a second worktree off it alters that. Verify it by applying the effect by hand in the worktree and **saying in the hand-back that the config path itself was never exercised**. Do **not** reach into the main checkout to make the helper see it early — that is the one thing that would work, and it is rejected in `/pipeline:execute`'s *Worktree creation*, which carries the argument for all of this. **Read the property as the one Step 4 stands on rather than as a bug**: Step 4 proves this pass's own config by cutting a real worktree *before* anything is committed, and it can only do that because the helper reads the working tree Step 2 wrote the file into. *The failure this prevents: a config change ships green having tested everything except itself, with nothing in the diff, the gate or the hand-back to tell it apart from one that was verified.*
