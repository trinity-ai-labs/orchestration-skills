...
**The tick is NOT how you learn an agent finished**, which arrives as a notification anyway; it catches a wandering agent mid-flight — **and what you do about what it catches is two levers rather than one, the destructive one second**, since most of what a tick surfaces is a wrong fact you correct by messaging the live agent and only an UNAUTHORIZED scope change warrants a stop — a path you yourself granted on a slice's ask is not one (`skills/glossary/vocabulary/divergence.md`). And **no reading of the timer tool's own description reaches this requirement**, its polling warning being about polling for completion. Interval, purpose and requirement are settled **here**, because an instruction reached only by a pointer is one a reader can skip while satisfying every step in front of them; each tick you snapshot every live worktree's diff against its **fork point**, drain the gate queue, **answer any question a live slice has asked you** — a channel nobody is told to listen on is not a channel, and left off that list every other step still goes green while the slice hands its question back cold — fast-forward the local integration branch, sweep for parked work, and — where an epic branch is live — merge the integration branch into it in the **epic's own worktree**, never the main checkout.
...
- ⛔ **The decide-don't-ask bar. Wave assignment, sizing, re-slicing and every decision to FOLD are this loop's calls to make and report**, not questions to put to the user: decide them, act, and record what you decided. **Filing is the exception, because it is the one disposition that leaves the loop's hands** — it spends a whole unit of work, a worktree and a PR and a gate, on something the arc will not carry, and it moves the reasoning to a person without the tree open. So a fold is reported; **a filing is put to the user with the reasoning and a recommendation**, at the same bar as any other escalation. Escalate exactly one class — **a product or design fork the code and conventions cannot settle** — in plain chat, **one question at a time, with your recommendation and why**. Two things are specific to a loop: **the bar is applied once per cycle, so setting it slightly too low multiplies**, and **this loop has two channels to the user** — most of what reaches one arrives as a *filed issue* rather than a question in chat, so a bar policed only over questions leaves the larger channel unguarded. **Read that multiplication off the cycle's record rather than off your sense of it** — it carries how many of the arc's filed items are still *Adjacent* and how many cycles the oldest has been, so a bar set slightly too low shows as a number while the pile is still small.
