...
| `upstreamFindings` | skills | `true` lets an arc's close-out file a **pipeline** finding as an issue against this plugin's own **public** repository. Absent, `false`, or anything that is not exactly `true` means **no** — a decided answer, and the opposite reading of absence to `sharedResources`. Moot in the plugin's own repository, where the target and the tree are one. See [Filing findings upstream](#filing-findings-upstream-off-by-default) |
