...
- **Title** + **branch name** with the right prefix (`feat/…`, `fix/…`, `refactor/…`, `docs/…`). Docs-only slices that match the config's `docsBranchPrefix` get the fast path — flag them.
...
- **Epic branch — answer it explicitly, every time, in one line.** An epic branch is a convergence branch cut from the integration branch that an epic's slices fork from and PR into, so the shared branch never carries the epic half-finished; `/pipeline:orchestrate` owns its lifecycle, and **you are the pass positioned to see whether it is warranted**, because you ground slices in real code and produce the seam map. The question is one: **does any intermediate state leave the integration branch in a condition you would not ship?** If yes, recommend one and name the unshippable state — the two shapes are a wave-0 foundational change every consumer must follow (the transient-red case above, where the branch is knowingly red for the whole run) and a **contract seam** whose halves must land together (below), which is the same condition arriving by another route. If no, say "not needed" and why, and the slices target the integration branch as usual. Answer it either way: an unanswered question reads as "no" without anyone having asked it. **Slice count is not the trigger, and neither is a busy integration branch** — a five-slice epic whose every slice is independently shippable wants no epic branch at all, and "other sessions are on that branch" is near-permanently true in a multi-session shop, so either one taken alone turns a narrow trigger into a default. You recommend; `/pipeline:orchestrate` cuts the branch, points the slices at it, and pays its cost (deferred conflicts, mitigated by merging integration → epic on its poll tick) — the same find/act split as the contract seam map.
...
- Epic branch: not needed — every intermediate state is independently shippable
...
> **Ready to orchestrate.** Hand this to `/pipeline:orchestrate` — e.g. `/pipeline:orchestrate work issue #<N>` (GH path) or "orchestrate this plan" (in-chat path). It will cut a worktree per slice, dispatch implementers wave by wave, review each PR, and merge.
...
- **Hand off, then stop.** Your turn ends at the breakdown + the handoff line. The user (or you, in a fresh `/pipeline:orchestrate` invocation) takes it from there.
