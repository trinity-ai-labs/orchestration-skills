...
**Parallelism has a price, and what you produce about it is a RECOMMENDED WAVE WIDTH — how many of the ready issues you argue should go out together — never a slice count you manufacture, and never the call itself**: you cannot see the queue, what is already live, or what the running host can take, so the width you name rides with the evidence only you hold and the dispatching seat decides it against the machine. Recommend the *fewest* that still expose the real independence (`skills/ground/references/slicing.md`'s *Sizing* carries the evidence to state with it, the axes a width is priced on, the principle behind the split, and the altitude it is measured at).
...
- **GitHub issue** (`ground #<issue>`) — read it with `gh issue view <N>`, ground it, then write the breakdown back to GitHub. Ask once if it's ambiguous between this and the in-chat path.
...
**Escalate only the gaps you genuinely can't resolve** — no obvious default, guessing wrong would change the wave or send an implementer down the wrong path, and the codebase and conventions don't settle it: a product decision, a design fork with no house style, an undefined acceptance bar that gates other slices. Don't ask what one more file would answer.
