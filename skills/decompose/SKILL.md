...
- The *scoped* per-commit check implementers run (Trinity: `pnpm check` — fast, unlocked, parallel) is NOT the sizing cost; the **drained full gate** is. If a project's full gate is genuinely cheap or parallelizable, this pressure relaxes and you can fan out wider — so always size against the *actual* `GATE_CMD` and drain model you read in config, not a default assumption.
