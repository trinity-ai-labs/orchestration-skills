...
| `scopedCheck` | skills | The cheap bar an implementer's commits are held to — by the repo's pre-commit hook where one runs it, and by the implementer before each commit where none does. Whether a hook runs it is read off the repository, never declared |
